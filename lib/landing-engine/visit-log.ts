...
export type VisitLogType = "websitevisit" | "subscribe" | "autoredirect";
...
export function sendVisitLog(type: VisitLogType, slug?: string): void {
  ...
  const isMobile =
    /Mobi|Android/i.test(navigator.userAgent) || navigator.maxTouchPoints > 0;

  const payload = {
    type,
    slug: slug || window.location.pathname.replace(/^\/lp\//, ""),
